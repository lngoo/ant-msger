...
/**
 * 发送到topic（to-topic）数据处理器
 */
public class SendToTopicMsgSubscriber implements MessageListener {
    private static final Logger LOG = LoggerFactory.getLogger("toTopicChannel");
    ...
    public void onMessage(Message message, byte[] bytes) {
        ...
        try {
            ...
            // 获取topicID
            String topicId = message1.getSendTo();
            Set<TopicUser> setTopicUsers = TopicManager.getInstance().getByTopicId(topicId);
            if (null == setTopicUsers || setTopicUsers.isEmpty()) {
                LOG.info("### topicId have 0 users. stop all. topicId={}", topicId);
                return;
            }
            ...
            for (TopicUser topicUser : setTopicUsers) {
                ...
                if (null == sessions || sessions.isEmpty()) {
                    LOG.info("### userId have 0 session. userId={}", topicUser.getUserId());
                    continue;
                }
